  ...
  async function sendVerificationEmail() {
    ...
    try {
      ...
      toast.success("Verification email sent", {
        description: "Open the link in your inbox when you're ready - you can keep using SANTRA without it.",
      });
    } catch (error) {
  ...
  if (!hydrated) {
    return (
      <div className="relative shrink-0" aria-hidden>
        <div className="h-9 w-9 rounded-2xl border border-white/10 bg-white/[0.04] md:h-[36px] md:w-[120px]" />
      </div>
    );
  }
  ...
  return (
    <div className="relative shrink-0">
      <button
        ref={triggerRef}
        onClick={toggleMenu}
        className="flex max-w-[200px] items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.06] p-2 text-xs text-white/60 transition hover:bg-white/[0.09] md:px-3 md:py-2"
      ...
      >
        ...
        <span className="hidden truncate md:inline">{profile.displayName || label}</span>
        <ChevronDown
          className={cn(
            "hidden h-3 w-3 shrink-0 transition md:block",
            open && "rotate-180",
          )}
        />
      </button>
