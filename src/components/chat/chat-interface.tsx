...
import { createMonitorFromText } from "@/lib/gtm/create-monitor-from-text";
...
const initialMessages: ChatMessage[] = [
  {
    ...
    content:
      "I am SANTRA AI on Strategy Desk. Ask about competitors, pricing risk, monitor signals, or draft an executive brief.",
  },
];
...
export function ChatInterface({
  ...
  const [watchingMessageId, setWatchingMessageId] = useState<string | null>(null);
  const {
    listening,
    transcribing,
    liveTranscript,
  ...
  function watchSourceForMessage(messageId: string) {
  ...
  async function watchThis(messageId: string) {
    const source = watchSourceForMessage(messageId).trim();
    if (!source) {
      toast.error("Nothing to watch yet.");
      return;
    }
    setWatchingMessageId(messageId);
    try {
      const monitor = await createMonitorFromText(source);
      toast.success("Monitor created — opening live check", {
        description: monitor.plainSummary || monitor.requirement,
      });
      window.location.href = `/alerts?runCheck=${encodeURIComponent(monitor.id)}`;
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not create monitor.");
    } finally {
      setWatchingMessageId(null);
    }
  }
  ...
  async function sendMessage(nextInput = input, options?: { fromLiveCall?: boolean }) {
    ...
    try {
      const payload = {
        ...
        // Live call: Research toggle controls heavy web collection (not forced on).
        useGtmAgent: useGtmAgentRef.current,
        fromLiveCall: Boolean(options?.fromLiveCall),
      };
  ...
  const body = (
    <>
      {chatMode === "unavailable" && (
        <p className={cn("mb-4 shrink-0 rounded-xl border border-amber-300/25 bg-amber-400/10 px-4 py-3 text-sm text-amber-100", !hideChrome && "-mt-2")}>
      ...
      >
        ...
        >
          {!hideChrome && (
            <div className="shrink-0 border-b border-white/10 px-4 py-2.5 md:px-6 md:py-3">
              <p className="text-sm text-white/50">Account context from Monitors is applied automatically.</p>
            </div>
          )}

          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain p-3 md:p-5">
                        ...
                        {message.id !== "welcome" && (
                          <div className="mt-3 flex flex-wrap gap-2 border-t border-white/[0.06] pt-3">
                            <Button
                              ...
                              disabled={watchingMessageId === message.id || loading}
                              onClick={() => void watchThis(message.id)}
                            >
                              <Radar className="h-3.5 w-3.5" />
                              {watchingMessageId === message.id ? "Creating…" : "Watch this"}
                            </Button>
                          </div>
                        )}
                      </>
                      ...
                      <div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          <Button
                            ...
                            variant="ghost"
                            size="sm"
                            className="h-8 rounded-full border border-white/10 bg-white/[0.03] px-3 text-xs text-white/70"
                            disabled={watchingMessageId === message.id || loading}
                            onClick={() => void watchThis(message.id)}
                          >
                            <Radar className="h-3.5 w-3.5" />
          ...
          <div className="shrink-0 border-t border-white/10 bg-sentra-panel/95 p-2.5 backdrop-blur-sm sm:p-3 md:p-4">
            <div className="mb-2 flex flex-wrap items-center gap-1.5 sm:mb-3">
              <Button
                ...
                size="sm"
                className={cn("h-8 rounded-full px-2.5 text-xs sm:px-3", liveCall.active && "ring-1 ring-rose-400/50")}
              ...
              >
                ...
                <span className="max-[360px]:hidden">{liveCall.active ? "End call" : "Live call"}</span>
              </Button>
              <Button
                type="button"
              ...
              >
                ...
                <span className="max-[360px]:hidden">{useGtmAgent ? "Research on" : "Research"}</span>
              </Button>
              ...
              <SuggestedPromptsMenu
                ...
                buttonLabel="Prompts"
                menuSubtitle="Quick strategy questions"
            ...
            {liveCall.active && (
              <div className="mb-3 rounded-xl border border-rose-300/25 bg-rose-400/[0.08] px-3 py-2.5 text-sm text-rose-50/90">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="hidden text-rose-100/50 sm:inline">·</span>
                  <span className="hidden text-xs text-rose-100/70 sm:inline">
                    Speak, pause for answers. Turn on Research for live web evidence.
                  </span>
                </div>
            ...
            <p
              className={cn(
                ...
                !listening && !transcribing && "max-sm:hidden",
              )}
            >
              {listening
                ? liveTranscript
