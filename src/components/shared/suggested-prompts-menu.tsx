...
export function SuggestedPromptsMenu({
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    const width = Math.min(352, Math.max(rect.width, Math.min(288, window.innerWidth - 32)));
          ...
          <AnimatePresence>
            {open ? (
              <motion.div
                key="suggested-menu"
                id={menuId}
                initial={{ opacity: 0, y: menuPosition.openUp ? -8 : 8 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: menuPosition.openUp ? -8 : 8 }}
                style={{
                  position: "fixed",
