...
export async function resolvePendingAction(
  ...
  return patchPendingAction(userId, id, { status });
}

/** Update proposedAction (pending only) and/or resolve status. */
export async function patchPendingAction(
  userId: string,
  id: string,
  patch: { status?: PendingAction["status"]; proposedAction?: string },
) {
  await ensureMongoReady();
  const db = await getDb();
  const existing = await db.collection<DbPendingAction & { user_id: string }>("pending_actions").findOne({
    user_id: userId,
    id,
  });
  if (!existing) throw new Error("Action not found.");

  const $set: Partial<DbPendingAction> = {};

  if (typeof patch.proposedAction === "string") {
    if (existing.status !== "pending") {
      throw new Error("Only pending actions can be edited.");
    }
    const trimmed = patch.proposedAction.trim();
    if (!trimmed) throw new Error("proposedAction cannot be empty.");
    $set.proposed_action = trimmed.slice(0, 2000);
  }
  ...
  if (!Object.keys($set).length) {
    const { user_id: _, ...mapped } = existing;
    return mapRow(mapped);
  }
