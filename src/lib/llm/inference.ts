...
export function isLlmQuotaError(error: unknown) {
  ...
  return (
    status === 402 ||
    status === 429 ||
    /insufficient|quota|credit|balance|billing|payment required|rate limit|exceeded/i.test(message)
  );
}
...
export function formatInferenceError(error: unknown) {
  ...
  return error instanceof Error ? error.message : "SANTRA AI could not generate a response.";
}
...
export async function createChatCompletionWithFallback(
  ...
  for (const entry of chain) {
    ...
    try {
      ...
      lastError = error;
      if (!isLlmAuthError(error) && !isLlmQuotaError(error)) throw error;
      console.warn(`[llm] ${entry.provider} unavailable, trying next provider`, getErrorMessage(error));
    }
  }
