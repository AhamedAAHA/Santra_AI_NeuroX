...
export function updateServerPendingAction(
  ...
  return patchServerPendingAction(userId, id, { status });
}

export function patchServerPendingAction(
  userId: string,
  id: string,
  patch: { status?: PendingActionStatus; proposedAction?: string },
): PendingAction | null {
  const row = store().get(id);
  if (!row || row.userId !== userId) return null;
  ...
  if (typeof patch.proposedAction === "string") {
    ...
    row.proposedAction = trimmed.slice(0, 2000);
  }
