...
export async function POST(request: Request) {
  try {
    ...
    const fromLiveCall = Boolean(body.fromLiveCall);
    const useGtmAgent = Boolean(body.useGtmAgent);
    ...
    // Live call defaults to fast path: skip Exa/Bright Data unless Research is on.
    const shouldRunCollection =
      !documentEvidence &&
      shouldCollectWebEvidence(message) &&
      (!fromLiveCall || useGtmAgent);
    ...
    const response = await generateChatResponse(message, {
      ...
      voiceMode: fromLiveCall,
      // Skip AIML live-search models on voice turns unless Research collected evidence.
      skipLiveSearch: fromLiveCall && !brightDataEvidence,
    });
