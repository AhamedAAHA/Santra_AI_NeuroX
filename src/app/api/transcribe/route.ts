...
async function transcribeUploadedAudio(file: File, contextText?: string, language = "en") {
  ...
  if (isSpeechmaticsSttConfigured()) {
    try {
      ...
      console.warn("Speechmatics STT failed, falling back to AIML/OpenAI", error);
    }
  }

  // AIML async STT, then OpenAI-compatible /audio/transcriptions fallback
  const text = await transcribeAudio(prepared, contextText);
