...
export async function GET() {
  ...
  try {
    const reports = await listIntelligenceReports(auth.user.id);
    return NextResponse.json({ reports });
  } catch (error) {
    console.warn("Reports collection unavailable", error);
...
export async function POST(request: Request) {
  ...
  try {
    const saved = await saveIntelligenceReport(auth.user.id, body.report, body.monitorId);
