...
/** AIML `/v1/stt/create` model ids (e.g. `#g1_whisper-medium`). OpenAI names like `whisper-1` are invalid here. */
export function getAimlSttModel() {
  const configured = process.env.AIML_MODEL_TRANSCRIBE?.trim();
  if (!configured) return "#g1_whisper-medium";
  // OpenAI-compatible transcription model names are not valid for AIML async STT.
  if (configured === "whisper-1" || configured.startsWith("gpt-4o") || configured.includes("transcribe")) {
    return "#g1_whisper-medium";
  }
  return configured;
}
