...
export type BrightDataEvidence = {
  ...
  query: string;
  evidence: string;
  collectionMode?: BrightDataMode | "exa";
  raw?: unknown;
  cacheHit?: boolean;
};
...
export type GtmEvidenceBundle = {
  ...
  query: string;
  targetUrl?: string;
  steps: Array<{ mode: BrightDataMode | "exa"; label: string; evidence: string }>;
  evidence: string;
...
async function readCache(cacheKey: string): Promise<BrightDataEvidence | null> {
  ...
  try {
    ...
    const data = await db.collection("bd_cache").findOne({ cache_key: cacheKey });

    if (!data || new Date(String(data.expires_at)) < new Date()) {
      if (data) await db.collection("bd_cache").deleteOne({ cache_key: cacheKey });
      return null;
    }
...
async function writeCache(cacheKey: string, value: BrightDataEvidence) {
  ...
  try {
    ...
    await db.collection("bd_cache").updateOne(
      { cache_key: cacheKey },
      { $set: { cache_key: cacheKey, payload: value, expires_at: expiresAt, created_at: new Date().toISOString() } },
      { upsert: true },
    );
  } catch (error) {
