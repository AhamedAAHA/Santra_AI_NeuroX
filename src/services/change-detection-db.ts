...
export async function runChangeDetectionWithDb({
  ...
  const previousSnapshot = await getLatestPageSnapshot(userId, monitorId, url);
  const changes = compareSnapshots(previousSnapshot ?? undefined, snapshot, monitorId);

  await savePageSnapshotDb(userId, snapshot);
  if (changes.length) {
    await saveDetectedChangesDb(userId, changes);
  }
