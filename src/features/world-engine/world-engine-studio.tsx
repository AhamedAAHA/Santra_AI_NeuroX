      ...
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        <div className="max-w-4xl">
          <div className="flex flex-wrap gap-2">
            <Badge variant="cyan">{report.scenarioMode ? "Simulation active" : "Startup intelligence active"}</Badge>
            <Badge variant={report.provider === "demo" ? "violet" : "success"}>
              {report.provider === "demo"
                ? "Illustrative demo signals"
                : report.provider === "openai-live"
                  ? "OpenAI intelligence"
  ...
  return (
    <Card className="relative self-start overflow-hidden p-4 md:p-5" glow>
      ...
      <div className="relative flex items-center justify-between">
        ...
        <Button variant="ghost" size="icon" onClick={onFullscreen} disabled={!signals.length} aria-label="Launch Scenario Pulse Mode">
...
function WorldPulseMode({
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 overflow-hidden bg-[#03050d]" role="dialog" aria-modal="true" aria-label="Scenario Pulse Mode">
      ...
      <div className="relative flex items-center justify-between border-b border-white/10 px-5 py-4 md:px-8">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="ghost" size="icon" onClick={onClose} aria-label="Close Scenario Pulse Mode"><X className="h-4 w-4" /></Button>
        </div>
      </div>
...
export function WorldEngineStudio({
  ...
  async function runWorldEngine(nextPrompt = prompt) {
    ...
    if (!settings.analyst.worldIntelligence) {
      toast.message("AI Startup Advisor is disabled in Settings.");
      return;
    }
    stopSpeechInput();
    setPrompt(query);
    ...
    try {
      ...
      toast.error("Competitor intelligence could not complete the brief.", {
    ...
    <>
      ...
      {loading && settings.analyst.liveLogs && (
        <Card className="mb-5 flex flex-wrap items-center justify-between gap-4 p-4 md:p-5" glow>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-white">Competitor Intelligence Center is synthesizing intelligence</p>
      ...
      {!settings.analyst.worldIntelligence && (
        <Card className="grid min-h-56 place-items-center p-8 text-center" glow>
          <div>
            ...
            <h2 className="mt-4 text-xl font-semibold text-white">Competitor intelligence mode is disabled</h2>
            <p className="mt-2 text-sm text-white/48">Enable AI Startup Advisor settings to launch competitor intelligence.</p>
          </div>
        </Card>
      )}
      ...
      {settings.analyst.worldIntelligence && !loading && !report && (
        <Card className="grid min-h-56 place-items-center p-8 text-center" glow>
          <div>
            <Globe2 className="mx-auto h-10 w-10 text-sentra-cyan" />
            <h2 className="mt-4 text-xl font-semibold text-white">Competitor Intelligence Center awaiting a directive</h2>
      ...
      {!loading && report && (
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} aria-label="AI Startup Advisor report">
          <Card className="p-6 md:p-8" glow>
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Button variant="ghost" className="justify-start" onClick={() => setPulseMode(true)} disabled={!report.signals.length}>
                <Expand className="h-4 w-4" /> Scenario Pulse mode
              </Button>
            </div>
          </Card>
        </motion.div>
