...
export function InvestigationStudio() {
  ...
  async function shareReport() {
    if (!report) return;
    const text = `SANTRA Visual Intelligence Verdict: ${report.status} (${report.scores.confidence}% confidence)\n${report.summary}`;
    try {
      if (navigator.share) {
        await navigator.share({ title: "SANTRA Investigation Report", text });
      } else {
