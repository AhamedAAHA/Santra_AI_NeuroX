# Web Data UNLOCKED - GTM Intelligence submission (SANTRA AI)
...
SANTRA AI is a GTM intelligence OS: Bright Data collects live competitor and market evidence, AI/ML API reasons over it, and monitors deliver executive briefs, alerts, and Speechmatics voice playback.

## Long description (template)

Enterprise GTM and strategy teams spend dozens of hours stitching competitor sites, SERPs, pricing pages, and hiring signals across disconnected tools. SANTRA AI unifies that workflow into one command center.

Users describe what to watch in plain language; SANTRA interprets monitor intent, collects evidence through **Bright Data SERP API** and **Web Unlocker**, and synthesizes risks, opportunities, and recommendations with **AI/ML API** (live search and analysis models). The **Dashboard** refreshes live briefings; **Chat** answers GTM questions with provider badges showing Bright Data + AIML when collection runs; **Alerts** run manual or **scheduled monitor checks** (Vercel cron every 30 minutes) and optional webhooks. **World Engine** and **Visual Forensics** extend investigations for macro and image-based cases.

SANTRA is built on Next.js 15, Supabase (auth + persistence + API key vault), and deploys to Vercel. Production disables sample Bright Data fallback so judges always see live web evidence when zones are configured.
...
| Time | Content |
|------|---------|
| 0:00–0:30 | Problem: fragmented GTM research; SANTRA as GTM OS |
...
## Slides (PDF for lablab)

**Ready-made deck:** open [`hackathon-presentation.html`](./hackathon-presentation.html) in Chrome → Print → Save as PDF (**A4 portrait**, no margins, scale 100%, background graphics on).  
**Editable outline:** [`HACKATHON_PRESENTATION_SLIDES.md`](./HACKATHON_PRESENTATION_SLIDES.md) for PowerPoint / Google Slides.

7 slides (3-min formula): Title → Problem → Solution → **Live Demo** → Pipeline → Future Impact → Close.  
Add screenshots on slides 4–5 (dashed placeholders). A4 portrait PDF.
