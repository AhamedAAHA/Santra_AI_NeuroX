...
SANTRA signs users in immediately after sign-up (no inbox step). In Supabase:

1. **Authentication → Providers → Email** - turn **off** “Confirm email” (recommended), or leave it on; SANTRA auto-confirms accounts on create so login still works.
